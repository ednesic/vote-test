...
func (s *server) createVote(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		defer s.logger.Info(voteCreateMsg, zap.Error(err), zap.Int32("electionId", vote.GetElectionId()), zap.String("Candidate", vote.GetCandidate()), zap.Int("StatusCode", stsCode))
	}()
